/**
 * @fileoverview Route-level integration tests for report email dispatch endpoint.
 */
...
require.cache[emailModulePath] = {
    ...
    exports: {
        sendTemplatedEmail: async (payload) => {
            ...
            return { accepted: [payload?.to], messageId: "test-email-report" };
        },
    },
};
...
async function seedPostedJournalEntry({ createdByUserId, approvedByUserId, entryDate, description, lines }) {
    ...
    const journalResult = await db.query(
        `INSERT INTO journal_entries
         ...
         RETURNING id`,
        [entryDate, description, totals.debits, totals.credits, createdByUserId, approvedByUserId, `JE-REPORT-EMAIL-${Date.now()}-${Math.floor(Math.random() * 100000)}`],
    );
...
test("manager can email trial balance report as CSV attachment", async () => {
    const manager = await insertUser({ username: "manager_reports_email_1", email: "manager_reports_email_1@example.com", role: "manager" });
    const accountant = await insertUser({ username: "acct_reports_email_1", email: "acct_reports_email_1@example.com", role: "accountant" });
    const managerToken = "manager-reports-email-token-1";
    ...
    const revenue = await insertAccount({
        userId: accountant.id,
        accountName: "Revenue Report Email",
    ...
    await seedPostedJournalEntry({
        ...
        description: "Report email revenue",
        lines: [
            { accountId: cash.id, dc: "debit", amount: 250 },
            { accountId: revenue.id, dc: "credit", amount: 250 },
        ],
    });
    ...
    try {
        ...
        assert.equal(response.body.messageCode, "MSG_EMAIL_SENT_SUCCESS");
        assert.equal(response.body.report_type, "trial_balance");

        assert.equal(emailCalls.length, 1);
        assert.equal(emailCalls[0].to, "finance@example.com");
        assert.equal(emailCalls[0].templateName, "direct_message");
        assert.ok(Array.isArray(emailCalls[0].attachments));
        assert.equal(emailCalls[0].attachments.length, 1);
        assert.match(emailCalls[0].attachments[0].filename, /trial_balance_\d{4}-\d{2}-\d{2}\.csv/);
        assert.ok(String(emailCalls[0].attachments[0].content).includes("account_number"));
    } finally {
        server.close();
...
test("accountant cannot email reports", async () => {
    const accountant = await insertUser({ username: "acct_reports_email_2", email: "acct_reports_email_2@example.com", role: "accountant" });
    const token = "acct-reports-email-token-2";
